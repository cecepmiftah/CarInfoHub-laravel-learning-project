...
class CarController extends Controller
{
    ...
    public function search(Request $request) 
    {
        ...
        $filters = [
            'maker_id', 'model_id', 'state_id', 'city_id', 'car_type_id', 'fuel_type_id',
            'year_from' => 'year', 'year_to' => 'year', 
            'price_from' => 'price', 'price_to' => 'price', 
            'mileage'
        ];

        foreach ($filters as $key => $value) {
            if (is_int($key)) {
                $cars->when($request->$value, fn ($q) => $q->where($value, $request->$value));
            } else {
                $operator = str_contains($key, 'from') ? '>=' : '<=';
                $cars->when($request->$key, fn ($q) => $q->where($value, $operator, $request->$key));
            }
        }
